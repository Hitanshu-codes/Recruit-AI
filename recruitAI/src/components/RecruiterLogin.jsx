  ...
  return (
    <div className='absolute top-0 left-0 bottom-0 right-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        <button className='bg-blue-600 w-full text-white py-2 rounded-full mt-4' type='submit'>{state === 'Login' ? 'Login' : isNextDataSubmitted ? 'Create Account' : 'Next'}</button>
        {
          state === 'Login' ?
            <p className='mt-5 text-center'>Don't have an account? <span onClick={() => setState('Signup')} className='text-blue-600 cursor-pointer'>
              Sign Up</span></p> :
        ...
        <img className='absolute top-4 right-4 cursor-pointer' onClick={e => setShowRecruiterLogin(false)} src={assets.cross_icon} alt="" />
      </form>
    </div>
  )
}
