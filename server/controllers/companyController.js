...
export const loginCompany = async (req, res) => {
  ...
  try {
    ...
    if (!company) return res.status(400).json({ success: false, message: "Company login failed because of invalid credentials" })
...
export const changeJobApplicationStatus = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: "Job application status changed successfully" })
  } catch (error) {
    console.log(error)
    return res.status(500).json({ success: false, message: error.message })
